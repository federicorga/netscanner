...
async function getRegister(domain, type) {
    ...
    try {
        ...
        if(result.Answer === undefined || result.Answer === null || result.Answer.length === 0) {
            ...
            return {
                success: false,
                message: `No se encontró registro ${type} para el dominio ${domain}.`,
                data: null,
                error: null,
                meta: { domain, type}
            }
        ...
        // Aquí formateamos el error y lo lanzamos para que lo atrape el caller
        throw new Error(`Error al consultar registros DNS ${type}: ${err.message}`);
    }
}
